...
public class MakeIsoImageService extends ServiceMBeanSupport {
    ...
    protected void process(MediaCreationRequest rq) {
        ...
        try {
            ...
            try {
                File filesetDir = rq.getFilesetDir();
                File dataDir = filesetDir.getParentFile().getParentFile();
                File isoDir = new File(dataDir, "iso");
                rq.setIsoImageFile(new File(isoDir, filesetDir.getName()
                        + _ISO));

...
public class SpoolDirService extends ServiceMBeanSupport 
    ...
    private static final int MOVE_COPY_BUFFER_SIZE = 8192;
    ...
    public boolean move(File src, File dest) {
        ...
        if (copy(src, dest, new byte[MOVE_COPY_BUFFER_SIZE]) && delete(src)) {
            if (log.isDebugEnabled())
                    log.debug("Success: M-MOVE(FALLBACK) " + src + " -> " + dest);
            return true;
        }
